...
const ActiveAssessment = () => {
  ...
  const currentResults = filteredAssessments.slice(indexOfFirstItem, indexOfLastItem);
  ...
  <button className="bg-blue-900 text-white px-4 py-2 rounded-md border border-blue-900 hover:bg-blue-800 flex items-center space-x-2 transition duration-300 ease-in-out transform hover:scale-105">
    Filter
      ...
      <table className="min-w-full bg-white">
        <thead className="bg-gray-50 text-12px">
          <tr>
            ...
            <th scope="col" className="py-4 px-6 border-b border-gray-200 text-left">
              Assessment Name
            </th>
            <th className="py-2 px-4 border-b border-gray-200 text-left">Job Title</th>
              <th className="py-2 px-4 border-b border-gray-200 text-left">Total Test</th>
              <th className="py-2 px-4 border-b border-gray-200 text-left">Duration</th>
              <th className="py-2 px-4 border-b border-gray-200 text-left">Created on</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200 text-14px">
          {currentResults.length > 0 ? (
            currentResults.map((assessment, index) => (
              <tr key={index} className="hover:bg-active-link-bg cursor-pointer transition duration-150 group">
               
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 group-hover:text-white">
                  {assessment.assessment_name}
                </td>
                <td className="py-2 px-4 border-b border-gray-200">{assessment.job_role_id}</td>
                  <td className="py-2 px-4 border-b border-gray-200">{calculateTotalTests(assessment.tests)}</td>
                  <td className="py-2 px-4 border-b border-gray-200">{assessment.assessment_time} min</td>
                  <td className="py-2 px-4 border-b border-gray-200">{new Date(assessment.created_date).toLocaleDateString()}</td>
                  <td className="py-2 px-4 border-b border-gray-200">{getStatus(assessment.is_active)}</td>
            ...
            <tr>
              <td colSpan="3" className="text-center px-4 py-4 border bg-yellow-100 text-yellow-700">
      ...
      <CSSTransition in={showDeleteConfirmation} timeout={300} classNames="fade" unmountOnExit>

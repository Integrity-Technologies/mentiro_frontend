...
const ViewTestResult = () => {
  ...
  const openHistoryModal = (assessments) => {
    if (assessments.length > 0) {
      ...
      setIsModalOpen(false); // Ensure main modal is closed
    }
  };
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50 text-12px">
          <tr>
            ...
            <th className="px-6 py-4 text-left font-bold text-gray-900 uppercase tracking-wider relative flex items-center">
        ...
        <tbody className="bg-white divide-y divide-gray-200 text-14px">
          {filteredResults.length === 0 ? (
            <tr>
              <td colSpan="5" className="text-center px-4 py-4 border bg-white-100 text-black-700">
                    ...
                    {dropdownVisible[`${candidate.id}-${index}`] && (
                      <div className="absolute right-0 z-10 mt-2 w-48 bg-white border border-gray-200 rounded-md shadow-lg group-hover:text-white">
                        <button
                          ...
                          onClick={() => openHistoryModal(assessment.tests)}
                        >
                          History of Attempts
                        </button>
                      </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50 font-roboto">
          <div className="bg-white rounded-lg shadow-lg p-4 w-1/2 relative max-h-screen">
            ...
            <div className="mb-4">
              ...
              <h4 className="font-md tracking-wide">{selectedAssessmentName}</h4>
            </div>
            
            <div className="mb-4">
              <label className="font-xs font-bold text-blue-900">Candidate Name:</label>
              <h4 className="font-md tracking-wide">{selectedCandidateName}</h4>
            </div>
                ...
                <tbody>
                  {selectedTests.length > 0 ? (
                    selectedTests.map((test, index) => (
                      <tr key={index} className="border-t mb-0">
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50 font-roboto">
          <div className="bg-white rounded-lg shadow-lg p-4 w-1/2 relative">
            ...
            <div className="flex flex-col mb-4">
              ...
              <p className="text-lg text-gray-700">{selectedAssessmentName}</p>
            </div>
            <div className="flex flex-col mb-4">
              <h3 className="text-xl font-semibold">Candidate Name</h3>
              <p className="text-lg text-gray-700">{selectedCandidateName}</p>
            </div>
            <div>
